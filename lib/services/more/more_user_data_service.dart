...
Future<UserDataResponse?> getUserName() async{ //TODO Error handling
  try{
    final response = await HTTPHelper.get(_urlGet); 
    if(response == null){ //No response was created -> FAIL
      return null;
    }
    int sc = response.statusCode; 
    switch(sc){ //check the HTTP status codes
      case 200: //OK
...
Future<bool> postUserName(MoreChangeNameEvent event) async{ //TODO Error handling
  ...
  try{
    ...
    if(response == null){ //No response was returned -> FAIL
      return false;
    }
    if(response.statusCode != 200){ //something went wrong on the Server Side -> FAIL
      return false;
    }
  }catch(e){ //An Exception was thrown -> FAIL
    return false; 
  }
